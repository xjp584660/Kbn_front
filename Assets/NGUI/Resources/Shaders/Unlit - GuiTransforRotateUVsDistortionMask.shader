...
Shader "Unlit/GuiTransforRotateUVsDistortionMask"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata_t v)
			{
				...
				float tmp = sin(_Time *_ScaleTimeInterval) * _Scale;
				float2x2 trans = rotationMatrix * ( 1 + tmp);
				v.texcoord.xy = mul ( v.texcoord.xy, trans);
				v.texcoord.xy += 0.5;
			...
			half4 frag (v2f IN) : COLOR
			{
				...
				half4 mask = tex2D( _MaskTex, IN.uvmask );
				bump*=mask.xy;
				half2 uvofs = bump*_Distortion;
				if (_ProjectionParams.x<0) uvofs.y = -uvofs.y;
				IN.texcoord.xy += uvofs;
				
				half4 col = tex2D (_MainTex, IN.texcoord + float2(_UOffset, _VOffset) * fmod(_Time.y,64)) * IN.color * _Color * mask;
