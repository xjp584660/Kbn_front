...
public class NetworkNode
{
    ...
    public void Close()
    {
		...
		{
			m_Socket.Close();
		 	//UnityEngine.Debug.LogWarning("CLose>>>>>>>>>");
	    }
    }
    ...
    /// <param name="userData">用户自定义数据。</param>
    public delegate void NetworkNodeConnectedHandler(NetworkNode sender, object userData);
    ...
    /// <param name="sender">触发事件的网络连接结点。</param>
    /// <param name="userData">用户自定义数据。</param>
    public delegate void NetworkNodeSendPacketHandler(NetworkNode sender, object userData);
    ...
    public delegate void NetworkNodeReceivePacketHandler(NetworkNode sender, int opcode, byte[] data);
	...
	public delegate void NetworkNodeErrorHandler(NetworkNode sender, NetworkErrorCode errorCode, string errorMessage);
