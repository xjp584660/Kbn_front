...
namespace ThinkingAnalytics
{
    ...
    public enum AUTO_TRACK_EVENTS
    {
        NONE = 0,
        /// <summary>
        /// 当应用进入前台的时候触发上报，对应 ta_app_start
        /// </summary>
        APP_START = 1 << 0,
        /// <summary>
        /// 当应用进入后台的时候触发上报，对应 ta_app_end
        /// </summary>
        APP_END = 1 << 1,
        /// <summary>
        /// 当出现未捕获异常的时候触发上报，对应 ta_app_crash
        /// </summary>
        APP_CRASH = 1 << 4,
        /// <summary>
        /// 应用安装后首次打开的时候触发上报，对应 ta_app_install
        /// </summary>
        APP_INSTALL = 1 << 5,
    ...
    public class ThinkingAnalyticsAPI : MonoBehaviour, TaExceptionHandler
    {
        ...
        public struct Token
        {
            ...
            /// <summary>
            ///  开启加密传输，默认false（仅支持iOS/Android）
            /// </summary>
            public bool enableEncrypt;
            /// <summary>
            /// // 密钥版本号（仅支持iOS/Android）
            /// </summary>
            ...
            /// <summary>
            /// // SSL证书验证模式，默认NONE（仅支持iOS/Android）
            /// </summary>
            public SSLPinningMode pinningMode;
            /// <summary>
            /// // 是否允许自建证书或者过期SSL证书，默认false（仅支持iOS/Android）
            /// </summary>
            public bool allowInvalidCertificates;
            /// <summary>
            /// // 是否验证证书域名，默认true（仅支持iOS/Android）
            /// </summary>
                    ...
                    for (int i=0; i<tokens.Length; i++) {
                        ...
                        if (!string.IsNullOrEmpty(token.appid)) {
                            ...
                            if (sInstances.ContainsKey(token.appid)) {
                                TD_Log.d("ThinkingAnalytics is repeated start with \nappId: "+token.appid);
                            }
                            else  {
                                token.appid = token.appid.Replace(" ", "");
                                TD_Log.d("ThinkingAnalytics start with \nappId: " + token.appid + "\nserverUrl: " + token.serverUrl + "\nmode: " + token.mode);
        ...
        public void Init() {
            taAPIInstance = this;
            Debug.LogWarning("<color=#5474F1FF>[ThinkingSDK]</color>---------- start init ThinkingAnalytics sdk  ----------.");
            ...
            Debug.LogWarning("<color=#5474F1FF>[ThinkingSDK]</color><color=#00ff00ff>---------- init ThinkingAnalytics sdk is OK ----------</color>.");
        ...
        private static readonly Dictionary<string, ThinkingAnalyticsWrapper> sInstances =  new Dictionary<string, ThinkingAnalyticsWrapper>();
